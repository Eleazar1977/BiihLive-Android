...
// 🔧 FIX: Configurar transporter de email usando AWS SES configurado
let transporter;
try {
    const config = functions.config();
    const awsSesConfig = config.aws_ses;

    if (awsSesConfig?.smtp_user && awsSesConfig?.smtp_password && awsSesConfig?.smtp_host) {
        transporter = nodemailer.createTransport({
            host: awsSesConfig.smtp_host,
            port: parseInt(awsSesConfig.smtp_port) || 587,
            secure: false, // true for 465, false for other ports
            auth: {
                user: awsSesConfig.smtp_user,
                pass: awsSesConfig.smtp_password
            }
        });
        console.log(`✅ Email transporter configurado con AWS SES: ${awsSesConfig.from_email}`);
    } else {
        throw new Error('AWS SES no configurado correctamente');
    }
} catch (error) {
    console.log('❌ Error configurando AWS SES:', error.message);
    // Fallback temporal
    transporter = {
...
exports.sendEmailVerificationCode = functions.https.onCall(async (data, context) => {
    try {
        ...
        console.log(`📊 [DEBUG] Guardando en "emailVerificationCodes" en base "basebiihlive" - FIXED DB CONNECTION`);
